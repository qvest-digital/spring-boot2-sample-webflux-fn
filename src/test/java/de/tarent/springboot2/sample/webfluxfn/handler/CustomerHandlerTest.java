...
public class CustomerHandlerTest {
	...
	void testFindCustomers() {
		webClient.get().uri("/customers/").accept(MediaType.APPLICATION_JSON_UTF8)
			.exchange()
			.expectStatus().isOk()
	...
	void testCreateInvalidCustomer() {
		...
		webClient.mutateWith(csrf()).post()
			...
			.expectStatus()
				.isBadRequest()
			.expectBody(JsonNode.class).consumeWith(exchange -> {
				JsonNode errors = exchange.getResponseBody();
